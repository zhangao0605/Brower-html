<template>
  <div class="mcd_con">
    ...
    >{{$t('title.blockchain_details')}} #{{all_data.height==undefined?is_height:all_data.height}}</div>
    <div class="mcd_all">
      <div class="mcd_all_con">
        <div class="mcd_all_con_left">{{$t('title.block_height')}}</div>
        <div class="mcd_all_con_right" style="  user-select: none;">
          {{all_data.height==undefined?is_height:all_data.height}}
          <span
            class="el-icon-caret-left mcd_icon"
            @click="block_switching(0)"
          ></span>
      ...
      <div class="mcd_all_con">
        <div class="mcd_all_con_left">{{$t('table.timestamp')}}</div>
        <div class="mcd_all_con_right">{{timestampToTime(all_data.timeTamp)}}</div>
      </div>
      <div class="mcd_all_con">
        <div class="mcd_all_con_left">{{$t('title.trading_information')}}</div>
        <div
          class="mcd_all_con_right"
          style="cursor: pointer"
          @click="to_transaction_details(all_data.height)"
        >
      ...
      <!--</div>-->
      <div class="mcd_all_con">
        <div class="mcd_all_con_left">Hash</div>
        <div class="mcd_all_con_right">{{all_data.hash}}</div>
      </div>
      <div class="mcd_all_con">
        <div class="mcd_all_con_left">{{$t('title.parent_block_hash')}}</div>
        <div
          class="mcd_all_con_right color_choose"
          @click="block_switching(0)"
        >{{all_data.previousHash}}</div>
      </div>
      <!-- ssss -->
      <div class="mcd_all_con">
        <div class="mcd_all_con_left">{{$t('title.packaging_node')}}</div>
        <div class="mcd_all_con_right">{{slice_address1(all_data.rewardAddress)}}</div>
      </div>
      <div class="mcd_all_con">
        <div class="mcd_all_con_left">{{$t('title.participation_round')}}</div>
        <div class="mcd_all_con_right">{{memberdetails.epoch}}</div>
      </div>
      <!-- <div class="mcd_all_con">
        <div class="mcd_all_con_left">{{$t('title.number_of_committees')}}</div>
        <div class="mcd_all_con_right">
      ...
      <div class="mcd_all_con">
        <div
          class="mcd_all_con_left mcd_all_con_left_last"
          style="line-height: 100px"
        >{{$t('title.participant_member_details')}}</div>
        <div class="mcd_all_con_right mcd_all_con_right_last">
          <!-- <span
            class="color_choose"
            v-for=" item in memberdetails.memberdetails"
            @click="to_address_details(item)"
          >{{item}}；&nbsp&nbsp&nbsp</span>-->
          <span v-for=" item in memberdetails.memberdetails">{{slice_address1(item)}}；&nbsp&nbsp&nbsp</span>
        </div>
      </div>
    </div>
  </div>
        ...
        getBlockDataByPage(data).then(response => {
          if (response.data.dataList.length == 0) {
            ...
            this.$store.dispatch("app/setCrosschainBlock_1", data).then(() => {
              ...
              getTxTypeByHeigh(data.chainId.toString(), this.is_height).then(
                response => {
                  if (response.data.length == 0) {
                    this.memberdetails = "";
                  } else {
...
<style scoped>
  ...
  .mcd_all_con_right_last::-webkit-scrollbar {
    /*滚动条整体样式*/
    width: 10px; /*高宽分别对应横竖滚动条的尺寸*/
    height: 1px;
  }

  .mcd_all_con_right_last::-webkit-scrollbar-thumb {
    /*滚动条里面小方块*/
    border-radius: 5px;
  ...
  .mcd_all_con_right_last::-webkit-scrollbar-track {
    /*滚动条里面轨道*/
