<template>
  <div class="ti_con">
    ...
    <div class="ti_search" style="width: 650px">
      <div class="ti_search1_2">
        <div class="ti_search2_1">
          <div class="ti_search2_1_input">
            <span class="el-icon-search ti_search2_1_input_icon"></span>
            <el-input v-model="search_transaction" :placeholder="$t('placeholder.placeholder_5')"></el-input>
          </div>
          <el-button type="primary" class="ti_search_submit" @click="search_hash()" style="float: right">
            {{$t('placeholder.search')}}
          </el-button>
        </div>
      </div>
    </div>
    <div class="ti_filter" style="width: 650px">
      ...
      <el-select style="margin-left: 40px" v-model="value1" :placeholder="$t('placeholder.choose')">
        <el-option
      ...
      <el-button type="primary" style="float: right" @click="filter_data()"> {{$t('placeholder.filter')}}</el-button>
    </div>
    <el-table
      :data="tableData"
      border
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <span class="to_tr show_color_choose"
                @click="see_transaction_details(scope.row.chainId,scope.row.txType,scope.row.hash)">{{slice_hash(scope.row.hash)}}</span>
        </template>
      </el-table-column>
      <el-table-column
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      currentPageChange(e) {
        ...
        if (this.is_click == true) {
          let data = {
            ...
            "txtype": this.value1.toString(),
            "pagesize": 10
          }
          getTxByTxTypeAndChainId(data).then(response => {
      ...
      see_transaction_details(id, type, hash) {
        let data = {
          'page': 1,
          'chainId': id.toString(),
          'hash': hash,
      ...
      filter_data() {
        this.is_click = true
        this.loading1 = true
        let data = {"chainId": this.value.toString(), "page": 1, "txtype": this.value1.toString(), "pagesize": 10}
        getTxByTxTypeAndChainId(data).then(response => {
          this.loading1 = false
          if (response.data.transactionsList.dataList.length == 0) {
            this.tableData = []
